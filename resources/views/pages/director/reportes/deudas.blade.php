    ...
    <div class="container py-5">
        ...
        <form method="GET" action="{{ route('dashboard.deudas') }}" class="mb-5">
                ...
                <div class="w-full md:w-1/3">
                    <select
                        ...
                        @foreach ($periodos as $periodoItem)
                            <option value="{{ $periodoItem->id }}"
                                {{ request('periodo') == $periodoItem->id ? 'selected' : '' }}>
                                {{ $periodoItem->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="w-full md:w-1/3">
                    <select
                        ...
                        @foreach ($grados as $gradoItem)
                            <option value="{{ $gradoItem }}" {{ request('grado') == $gradoItem ? 'selected' : '' }}>
                                {{ $gradoItem }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="w-full md:w-1/3">
                    <select
                        ...
                        @foreach ($secciones as $seccionItem)
                            <option value="{{ $seccionItem }}" {{ request('seccion') == $seccionItem ? 'selected' : '' }}>
                                {{ $seccionItem }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="flex gap-1 justify-center p-2">
                <button type="submit"
                    class="w-full md:w-auto px-4 py-2 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Filtrar</button>
